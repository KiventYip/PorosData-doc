...
> - updated_at: 2026-03-18 09:19:22
...
```30:30:data/structured/full_text/00001/00001_structured.txt
...
- `processed` 命中样本：`00001`
- `structured` 命中样本：`00001/full_text`、`00001/multimodal`
...
- `processed` 中不再出现 `\begin{array} ... \end{array})` 这类“环境+外挂右括号”结构；
- `structured` 中对应样本恢复为正常括号公式或安全降级文本。
...
```21:21:data/structured/full_text/00001/00001_structured.txt
<poros_paragraph>... rapidly quenched $\mathrm{\DeltaNi - Nb {- Y}}$ alloys is reported. ...</poros_paragraph>
```

同类残留：

```25:25:data/structured/full_text/00001/00001_structured.txt
...
- `processed` 中不再出现 `\DeltaNi` / `\DeltaNb` 作为材料式前缀；
- `structured` 中对应位置恢复为正常材料式，而不是保留脏 token。
...
```12:12:data/structured/full_text/00011/00011_structured.txt
...
- `processed` 明确命中样本：`00001`、`00011`、`00013`、`00020`
- `structured` 当前仍可见残留样本：`00001`、`00011`、`00020`
...
- `processed` 中不再出现 `$\mathbf{Ni -}$`、`$\mathrm{Zr -}$` 这类半截材料式；
- `structured` 中不再出现以 `<poros_equ>` 或裸 `$...$` 形式残留的半截合金 token。
...
这些属于正常物理量符号，在 `processed` 与 `structured` 中大量存在，不能用简单的“见 `\Delta` 就删”规则处理。
...
建议将本清单作为跨包修复输入，和 `docs/audit/designer_remediation_workitems_2026-03-17_000000.md` 配套使用：
